...
execute as @e[type=sheep,nbt={HurtTime:10s}] at @s run effect give @s minecraft:speed 10 0 true

# Fury
execute as @e[type=sheep] at @s if entity @a[distance=..8,scores={DmgDealt=1..}] run tag @e[type=sheep,distance=..8,nbt={Age:0}] add entityFury
